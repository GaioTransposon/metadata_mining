...
log_dir = "./logs/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + '_' + str(subsampling)

# Define training arguments
training_args = TrainingArguments(
    output_dir='./results',          
    num_train_epochs=3,              # increase the number of epochs for better visualization
...
# ***** Running training *****
#   Num examples = 160 # total number of examples in the training dataset.
#   Num Epochs = 3 # number of times the whole training dataset will be used to update the model.
#   Instantaneous batch size per device = 16 # number of examples used in each update (this is called a batch). Since you are likely training on a single device (probably a CPU or a single GPU), this is also the total batch size.
#   Total train batch size (w. parallel, distributed & accumulation) = 16
#   Gradient Accumulation steps = 1
#   Total optimization steps = 30 # total number of updates that will be performed. This is calculated as (Num examples / batch size) * Num Epochs, rounded up.
# <IPython.core.display.HTML object>
# ***** Running Evaluation ***** # model is being evaluated on the validation set.
#   Num examples = 40
#   Batch size = 64
# 
# Precision and F-score are ill-defined and being set to 0.0 in labels with no predicted samples. Use `zero_division` parameter to control this behavior.
# # warning from scikit-learn, which is used to calculate the precision and F-score. It's warning you that some of your classes have no predicted samples, and so the precision and F-score for those classes are undefined (and are being set to 0.0). This can happen if your model is not predicting certain classes at all, which might be a sign that your model is not training properly, perhaps due to class imbalance or other issues.
...
# Usage:
texts = ["This is about capitalism", "Another sample sentence."]
