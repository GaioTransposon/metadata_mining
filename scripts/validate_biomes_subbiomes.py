...
# CHANGED: read 3rd column as test_type (used for stats grouping). If the file has only 2 cols, this still works.
df_map = pd.read_csv(
...
# CHANGED: map each label to the FIRST filename it appears with in the TSV (deterministic, handles duplicate labels)
...
# CHANGED: compare only files within the same test_type (as listed in the TSV, in order)
...
# CHANGED: enforce TSV order by unique filename (stable sort)
